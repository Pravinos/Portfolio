...
![Groq](https://img.shields.io/badge/Groq-GPT--OSS--120B-4ade80?style=flat-square)

## Why I built it this way

I wanted a portfolio that felt like mine - not a generic template. The terminal aesthetic (`#4ade80` on `#0a0a0a`, monospace-style section labels like `// about`, command-row contact links) matches how I actually work: dark editor, green accents, code-first presentation.

Everything lives on one scrollable page. Recruiters and collaborators can skim the full picture in one pass, and the nav uses `IntersectionObserver` to highlight whichever section is in view. On mobile, the full navigation is available through a compact menu.
...
Next.js 16 App Router, React 19, TypeScript, Tailwind CSS v4, and Framer Motion for animations. The AI layer uses the Vercel AI SDK with Groq (`openai/gpt-oss-120b`). Typography is [Space Grotesk](https://fonts.google.com/specimen/Space+Grotesk) - used for both body and mono-style labels.
...
Colour tokens live in `tailwind.config.ts`: near-black background (`#0a0a0a`), elevated surfaces (`#111111` / `#1a1a1a`), green primary (`#4ade80`), and blue secondary (`#0ea5e9`) for highlights like language bars. Section entrances use Framer Motion `whileInView` so animations fire once on scroll, not on every re-render.
...
`POST /api/chat` validates and bounds conversation payloads, rate-limits to 10 requests per minute per IP using Upstash Redis in production, injects CV context as the system prompt, and streams via Groq. Local development uses an in-memory limiter. The widget uses `@ai-sdk/react`’s `useChat` hook with starter questions to reduce blank-page friction.

Model and token limits are configured in `lib/groq.ts`.

### Analytics and consent

Google Analytics and Vercel Analytics load only after explicit opt-in. A consent banner and footer “cookie preferences” control persist the choice in local storage; `trackEvent` no-ops until consent is granted. Events cover contact clicks and chat interactions. A `/privacy` page documents the setup under GDPR.
...
Hosted on Vercel at [portfolio.prav1nos.me](https://portfolio.prav1nos.me). Environment variables on the host: `GROQ_API_KEY` for the chat API, `NEXT_PUBLIC_SITE_URL` for canonical URLs, `NEXT_PUBLIC_GA_ID` for Google Analytics, and `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` for distributed chat rate limiting.
